...
import com.rcplatform.rclockscreen.Bean.Point;
import com.rcplatform.rclockscreen.utils.MathUtil;
...
public class PatternView extends View {
	...
	private boolean isPathVisiable = true;
...
//			if ((dx > 1 || dy > 1) && (dx != 2 * dy) && (dy != 2 * dx)) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			mCompleteListener.onTouchMoveing();
			if (checking) {
				p = checkSelectPoint(ex, ey);
				if (p == null) {
					movingNoPoint = true;
		...
		if (isFinish) {
			if (this.sPoints.size() == 1) {
				...
				error();
				clearPassword();
				Toast.makeText(this.getContext(), "至少绘制4个点！",
	...
	//路径是否可见
	public void setPathVisiable(boolean isPathVisiable){
		this.isPathVisiable = isPathVisiable;
	}
