...
public class CheckedRelativeLayout extends RelativeLayout{
    ...
    public CheckedRelativeLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        view = View.inflate(context, R.layout.check_view, null);
        intitView();
    ...
    private void intitView() {
        mContent = (TextView) view.findViewById(R.id.tv_check_content);
        mCheckBox = (CheckBox)view.findViewById(R.id.cb_check_img);
        mCheckBox.setChecked(attr_checked);
        if(!TextUtils.isEmpty(attr_content)){
            mContent.setText(attr_content);
        }
    }

    public void setBoxChecked(boolean isChecked){
